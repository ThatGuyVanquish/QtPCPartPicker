...
PSU::PSU(QString model, QString modularity, Efficiency efficiency, int wattage, int price, bool fan):
    _model(model),
...
void PSU::couldPower(CPU *cpu, GPU *gpu)
{
    double eff = effPercentage();
    _canPower = (cpu->getTDP() + gpu->getTDP()) <= _wattage * eff;
}

void PSU::couldPower(QList<CPU *> cpus, GPU *gpu)
{
    double eff = effPercentage();
    int powerUsage = gpu->getTDP();
